...
my $first_word_pattern = join("|", keys %words);
my $first_word_re = qr/$first_word_pattern/;

my $last_word_pattern = join("|", map { scalar reverse($_) } keys %words);
my $last_word_re = qr/$last_word_pattern/;
...
while (<DATA>) {
	...
	my $fd = $& if /\d/;
	my $fdo = defined $fd ? $-[0] : 1000;
	my $fw = $words{$&} if $_ =~ $first_word_re;
	my $fwo = defined $fw ? $-[0] : 1000;
	my $first = $fdo < $fwo ? $fd : $fw;

	$_ = scalar reverse $_;
	my $ld = $& if /\d/;
	my $ldo = defined $ld ? $-[0] : 1000;
	my $lw = $words{scalar reverse $&} if $_ =~ $last_word_re;
	my $lwo = defined $lw ? $-[0] : 1000;
	my $last = $ldo < $lwo ? $ld : $lw;
